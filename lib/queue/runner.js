...
Runner.prototype.stop = function (fn) {
  ...
  // NOTE: We may not shut down the queue in the test environment because
  // we use it as a singleton which is then used by the runner AND the http
  // server. If we'd shut the queue down here, we would also accidnelty shut
  // down the queue instance used in the http server.
